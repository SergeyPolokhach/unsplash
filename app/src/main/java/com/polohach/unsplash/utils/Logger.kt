...
object Logger {
    ...
    private fun callerTag(callStackLevel: Int = 0): String {
        ...
        do {
            ...
            if (matcher.find()) tag = matcher.replaceAll(EMPTY_STRING)
...
fun <T> T?.printLogE(text: String? = EMPTY_STRING, callLevel: Int = 1) = this.apply {
    Logger.run {
        e(message = { "$text ${this@printLogE}" }, callStackLevel = callLevel)
    }
}

fun <T> T?.printLog(text: String? = EMPTY_STRING, callLevel: Int = 1) = this.apply {
    Logger.run {
